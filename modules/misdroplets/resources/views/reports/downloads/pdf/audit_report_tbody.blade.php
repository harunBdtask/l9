      ...
      <tr @if($print) style="font-size: 7px !important" @endif>
        ...
        @php          
          ...
          if(isset($report->bundleCards->first()->cutting_date)) {
            ...
            $parties = \SkylarkSoft\GoRMG\Printembrdroplets\Models\PrintInventoryChallan::getPurchaseOrderWiseParties($report->purchase_order_id);
          }
        @endphp       
        ...
        @endphp
        <td title="@foreach ($parties as $party) {{ $party }} @if(!$loop->last),@endif @endforeach"> @if(!$print){{ substr($parties[0] ?? '', 0, 10) }} @else {{ implode(',', $parties) }}  @endif
        </td>
