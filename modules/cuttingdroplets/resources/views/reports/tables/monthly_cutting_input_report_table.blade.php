<table class="reportTable">
    <thead>
    <tr>
        <td style="background-color: aliceblue" rowspan="2"><b>DATE</b></td>
        ...
        <td rowspan="2" style="background-color: #cfe8fd"><b>REMARKS</b></td>
    </tr>
    <tr>
    ...
    @foreach($report as $key => $data)
        <tr>
            @if(\Carbon\Carbon::parse($key)->dayOfWeek == \Carbon\Carbon::FRIDAY)
                <td style="background-color: cadetblue;color: white;">
                    <b>{{ \Carbon\Carbon::make($key)->format('d-m-Y') }}</b></td>
                <td style="background-color: cadetblue;color: white;"
                    colspan="{{ collect($cuttingFloors)->count() + (collect($printEmbrFloors)->count()*4)
                                + collect($inputFloors)->count() + 7 }}">
                    <b>FRIDAY</b>
                </td>
            @else
                <td>{{ \Carbon\Carbon::make($key)->format('d-m-Y') }}</td>
                @foreach($cuttingFloors as $floor)
