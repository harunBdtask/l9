...
<table class="reportTable" id="fixTable" style="border-collapse: collapse;">
	...
	<tr>
		...
		<th style="width: 10%;">Order/Style</th>
		<th style="width: 18%;">Floor Line</th>
			...
			@foreach($reportByOrder->groupBy('color_id') as $reportByColor)
				@php
						...
						$reportByColor->groupBy('floor_id')->each(function ($floor_group, $floor_id) use(&$floor_line) {
							$floor_line .= $floor_group->first()->floor->floor_no.'<b>['.$floor_group->unique('line_id')->implode('line.line_no',',').']</b><br>';
						});
				...
				<tr>
					...
					<td style="background-color: #a8f5ff;">{!! $floor_line !!}</td>
					<td>{{ $reportByColor->first()->color->name }}</td>
					<td>{{ $order_excess_cut_qty }}</td>
					<td style="background-color: #cbffb5;">{{ $total_cutting_qty }}</td>
					<td>{{ $c_balance }}</td>
