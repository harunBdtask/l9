...
class BundleCardColorSizeWiseIrisFabricSerialHandler
{
    ...
    private function getStartingSerial($ratio)
    {
        if ($this->bundleCardGenerationDetail->is_tube == 1) {
            $serial = $this->bundleCardGenerationDetail->roll_summary['total_ply'];
        } else {
            $serial = $this->bundleCardGenerationDetail->roll_summary['total_ply'] * $ratio['ratio'];
        }
    ...
    public function handle()
    {
        ...
        foreach ($ratios as $ratio) {
            ...
            $startingSerial[$ratio['size_name']] += $this->getStartingSerial($ratio);
