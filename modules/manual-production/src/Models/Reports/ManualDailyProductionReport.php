...
class ManualDailyProductionReport extends Model
{
    use HasFactory;

    protected $table = "manual_daily_production_reports";
    ...
    public static function getInputReadyWipDataToDate($date)
    {
        $query = self::query()->whereDate('production_date', '<=', $date);
        return $query->sum('cutting_qty') - $query->sum('input_qty');
    }
}
