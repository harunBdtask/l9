...
@section("title","Embellishment Floor")
@section('content')
<div class="padding">
  <div class="box" >
    <div class="box-header">
      <h2>Embellishment Floor</h2>
    </div>

    <div class="box-body">
      <div class="row">
        <div class="col-sm-3 col-sm-offset-9">
          <form action="{{ url('subcontract-embellishment-floor') }}" method="GET">
      ...
      <div class="row m-t">
        <div class="col-sm-12 col-md-4">
          @if(getRole() == 'super-admin' || getRole() == 'admin' || Session::has('permision_of_embellishment_floor_add'))
          <form action="{{ url('subcontract-embellishment-floor') }}" method="post" id="form">
            @csrf
            ...
            <div class="form-group">
              <div class="text-center">
                ...
                <a href="{{ url('subcontract-embellishment-floor') }}" class="btn btn-sm btn-warning"><i
          ...
          <table class="table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>{!! $statusDom !!}</td>
                <td>
                  @if(getRole() == 'super-admin' || getRole() == 'admin' || Session::has('permision_of_embellishment_floor_edit'))
                  <a href="javascript:void(0)" data-id="{{ $item->id }}" class="btn btn-xs btn-warning edit"><i
                      class="fa fa-edit"></i></a>
                  @endif
                  @if(getRole() == 'super-admin' || getRole() == 'admin' || Session::has('permision_of_embellishment_floor_delete'))
                  <button type="button" class="btn btn-xs {{ $buttonClass }} status-update-modal" data-toggle="modal"
                    ...
                    data-url="{{ url('subcontract-embellishment-floor/'.$item->id."/status-update") }}"
...
<script>
  $(document).on('click', '.edit', function () {
      ...
      $.ajax({
          method: 'get',
          url: '{{ url('subcontract-embellishment-floor') }}/' + id + '/edit',
          success: function (result) {
              $('#form').attr('action', `subcontract-embellishment-floor/${result.id}`).append(`<input type="hidden" name="_method" value="PUT"/>`);
