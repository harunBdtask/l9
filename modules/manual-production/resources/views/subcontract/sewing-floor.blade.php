...
@section("title","Sub. Sewing Floor")
@section('content')
<div class="padding">
  <div class="box" >
    <div class="box-header">
      <h2>Subcontract Sewing Floor</h2>
    </div>

    <div class="box-body">
      <div class="row">
        <div class="col-sm-3 col-sm-offset-9">
          <form action="{{ url('subcontract-sewing-floor') }}" method="GET">
      ...
      <div class="row m-t">
        <div class="col-sm-12 col-md-4">
          @if(getRole() == 'super-admin' || getRole() == 'admin' || Session::has('permission_of_sewing_floor_add'))
          <form action="{{ url('subcontract-sewing-floor') }}" method="post" id="form">
            @csrf
            ...
            <div class="form-group">
              <div class="text-center">
                ...
                <a href="{{ url('subcontract-sewing-floor') }}" class="btn btn-sm btn-warning"><i
          ...
          <table class="table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>{!! $statusDom !!}</td>
                <td>
                  @if(getRole() == 'super-admin' || getRole() == 'admin' || Session::has('permission_of_sewing_floor_edit'))
                  <a href="javascript:void(0)" data-id="{{ $item->id }}" class="btn btn-xs btn-warning edit"><i
                      class="fa fa-edit"></i></a>
                  @endif
                  @if(getRole() == 'super-admin' || getRole() == 'admin' || Session::has('permission_of_sewing_floor_delete'))
                  <button type="button" class="btn btn-xs {{ $buttonClass }} status-update-modal" data-toggle="modal"
                    ...
                    data-url="{{ url('subcontract-sewing-floor/'.$item->id.'/status-update') }}"
...
<script>
  $(document).on('click', '.edit', function () {
      ...
      $.ajax({
          method: 'get',
          url: '{{ url('subcontract-sewing-floor') }}/' + id + '/edit',
          success: function (result) {
              $('#form').attr('action', `subcontract-sewing-floor/${result.id}`).append(`<input type="hidden" name="_method" value="PUT"/>`);
