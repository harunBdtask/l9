            ...
            <div class="box-body">
                ...
                <div class="row">
                    <div class="col-md-12 m-b">
                        <a href="{{ url('dyeing/compactors/create') }}"
                ...
                <div class="row">
                    <div class="col-md-12">
                        <table class="reportTable">
                            ...
                            <tbody>
                            {!! Form::open(['url'=>'/dyeing/compactors', 'method'=>'GET']) !!}
                            <tr>
                                <td>*</td>
                                <td>
                            ...
                            @forelse ($compactors as $compactor)
                                @php
                                    $machines = collect($compactor->dyeingBatch->machineAllocations)
                                                        ->pluck('machine.name')
                                                        ->implode(',');
                                @endphp
                                <tr>
                                    ...
                                    <td style="text-transform: capitalize">{{ $compactor->dyeingBatch->fabricSalesOrder->booking_type??null  }}</td>
                                    <td>{{ $compactor->buyer->name }}</td>
                                    <td>{{ $compactor->textile_order_no }}</td>
                                    <td>{{ $compactor->dyeing_batch_no }}</td>
                                    <td>{{ $machines }}</td>
                                    <td>{{ $compactor->shift->shift_name }}</td>
                                    <td>{{ $compactor->compactor_details_sum_finish_qty }}</td>
                                    <td>
                                        <a class="btn btn-xs btn-info" type="button"
                                           href="{{ url('dyeing/compactors/create?id='.$compactor->id) }}">
                                        ...
                                        <button style="margin-left: 2px;" type="button"
                                                ...
                                                ui-target="#animate"
                                                data-url="{{ url('dyeing/compactors/'.$compactor->id) }}">
