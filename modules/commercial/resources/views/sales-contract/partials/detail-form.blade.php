<table class="reportTable detail-form-table" style="width: 100%">
    ...
    @foreach($purchaseOrders as $key => $po)
        ...
        @php
            $detail = \SkylarkSoft\GoRMG\Commercial\Models\SalesContractDetail::where('po_id', $po->id)->first();
            ...
            $salesContractAttachValue = isset($po->salesContracts) ? collect($po->salesContracts)->where('sales_contract_id', '!=', $sales_contract_id)->sum('attach_value') : 0;
        @endphp
        ...
        <tr class="detail-tr">
            ...
            <td>
                {!! Form::number('attach_value[]', $attach_value, ['class' => 'form-control form-control-sm form-control form-control-sm-sm', 'step' => '0.01']) !!}
                <span class="text-danger attach_value_error"></span>
            </td>
            <td>{{ $salesContractAttachValue ?? 0 }}</td>
