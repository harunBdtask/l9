<Table class="reportTable detail-form-table" style="width: 100%">
    ...
    @foreach($purchaseOrders as $key => $po)
        ...
        @php
            ...
            $exportLCAttachValue = isset($po->exportLCs) ? collect($po->exportLCs)->where('export_lc_id', '!=', $export_lc_id)->sum('attach_value') : 0;
        ...
        <tr class="detail-tr">
            ...
            <td>
{{--                {!! Form::number('attach_value[]', $attach_value, ['class' => 'form-control form-control-sm form-control form-control-sm-sm', 'step' => '0.01','max' => $po->po_quantity * $po->avg_rate_pc_set]) !!}--}}
                {!! Form::number('attach_value[]', $attach_value, ['class' => 'form-control form-control-sm form-control form-control-sm-sm']) !!}
                <span class="text-danger attach_value_error"></span>
            </td>
            <td>{{ $exportLCAttachValue ?? 0 }}</td>
            <td>{{ $po->order->style_name }}</td>
            <td>{{ $po->poDetails->implode('garmentItem.name',',') }}</td>
