...
class PerformanceReportService
{
    ...
    public function report(): array
    {
        ...
        $orderFobValue = $order ? $order->purchaseOrders->map(function ($poCollection) {
            return $poCollection->po_quantity * $poCollection->avg_rate_pc_set;
        })->sum() : 0;

        $budgetCosting = isset($budget) ? $budget->costing : [];

        $reportData['style'] = $budget['style_name'] ?? '';
        $reportData['buyer'] = $budget->buyer->name ?? '';
        $reportData['company'] = $budget->factory->factory_name ?? '';
        $reportData['total_qty'] = $order['pq_qty_sum'] ?? 0;
        ...
        $reportData['budgetCommercialCost'] = isset($budget) ? ($budget->costing['comml_cost']['budgeted_cost'] / 12) * ($order['pq_qty_sum'] ?? 0) : 0;

        $reportData['budgetOthersCosting'] =  isset($budget) ? ($budgetCosting['cm_cost']['budgeted_cost']
            ...
            + $budgetCosting['certificate_cost']['budgeted_cost']) : 0;

        $reportData['budgetOthersCosting'] = isset($budget) ? (($reportData['budgetOthersCosting'] / 12) * ($order['pq_qty_sum'] ?? 0)) : 0;
        return $reportData;
    }
