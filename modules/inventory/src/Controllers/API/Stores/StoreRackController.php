...
use SkylarkSoft\GoRMG\Inventory\Requests\StoreRackRequest;

class StoreRackController extends Controller
{
    public function store(Store $store, StoreFloor $floor, StoreRoom $room, StoreRackRequest $request): JsonResponse
    {
        try {
            DB::beginTransaction();
            $id = $request->input('id');
            $message  = '';
            if ($id) {
                $room->racks()->find($id)->update($request->all());
                $message  = 'Data updated successfully!';
            } else {
                $room->racks()->create($request->all());
    ...
    public function destroy(StoreRack $rack): JsonResponse
    {
        try {
            DB::beginTransaction();
