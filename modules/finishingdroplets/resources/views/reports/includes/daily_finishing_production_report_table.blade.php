    ...
    @foreach($reports->groupBy('floor_id') as $reportByFloor)
        ...
        @foreach($reportByFloor->sortBy('line_sort') as $report)
            @php
                ...
                $style = $report->order->order_style_no;
                $po = $report->purchaseOrder->po_no;
                $po_qty = $report->purchaseOrder->po_quantity;
                $color = $report->color->name;
                $items = '';
                $report->order->items->each(function ($value, $key) use(&$items) {
                    $items .= $value->item->item_name.' ';
                });
                $delivery_date = $report->purchaseOrder->ex_factory_date ? date('d-M-Y', strtotime($report->purchaseOrder->ex_factory_date)) : '';
                $input_date_fetch = \SkylarkSoft\GoRMG\Inputdroplets\Models\FinishingProductionReport::poColorLineWiseFirstInputDate($report->purchase_order_id, $report->color_id, $report->line_id);
                ...
                $line_mp = '';
                $remarks = '';
                $sewing_line_target_query = \SkylarkSoft\GoRMG\Inputdroplets\Models\SewingLineTarget::whereDate('target_date', $date)->where([
                ...
                $quantity_fetch_related_query = \SkylarkSoft\GoRMG\Cuttingdroplets\Models\DateAndColorWiseProduction::whereDate('production_date', '<=', $date)->where([
                    'purchase_order_id' => $report->purchase_order_id,
                ...
                $total_line_input = \SkylarkSoft\GoRMG\Inputdroplets\Models\FinishingProductionReport::poColorLineWiseTotalInputQty($report->purchase_order_id, $report->color_id, $report->line_id, $date);
                $total_line_output = \SkylarkSoft\GoRMG\Inputdroplets\Models\FinishingProductionReport::poColorLineWiseTotalOutputQty($report->purchase_order_id, $report->color_id, $report->line_id, $date);
                $wip = $total_line_input - $total_line_output;
                $target_vs_achieve = $today_output - $target;
                $smv = $report->purchaseOrder->smv;
                $day_line_efficiency = 0;

                if(isset($report->purchaseOrder->smv)
                    && ($today_output > 0)
