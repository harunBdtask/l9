<table class="reportTable">
    ...
    <tr>
        ...
        <td rowspan="2"><strong>PACK</strong></td>
        ...
        <td rowspan="2"><strong>TTL Color Ratio(CTN)</strong></td>
        <td rowspan="2"><strong>TTL Pack Ratio(CTN)</strong></td>
        <td rowspan="2"><strong>SIZE WISE TTL QTY</strong></td>
    ...
    @foreach($garmentPackingProduction->carton_details as $cartonData)
        @foreach($cartonData['color_size_ratio'] as $colorSizeRatio)
            ...
            <tr>
                ...
                @if($loop->first)
                    <td rowspan="{{ $colorSizeRatioLen }}">
                        {{ $cartonData['total_qty_in_carton'] }}
                    </td>
                @endif
                <td>
                    {{ $colorSizeRatio['total_color_qty_in_carton'] }}
                </td>
                @if($loop->first)
                    <td rowspan="{{ $colorSizeRatioLen }}">
    ...
    <tr>
        ...
        <td colspan="{{ 6+collect($garmentPackingProduction->sizes)->count() }}"></td>
        <td>
