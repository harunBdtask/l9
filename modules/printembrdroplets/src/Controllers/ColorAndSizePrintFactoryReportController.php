...
use SkylarkSoft\GoRMG\Printembrdroplets\Exports\DateWisePrintFactoryReportExport;
...
class ColorAndSizePrintFactoryReportController extends Controller
{
    ...
    public function getBuyerWisePrintSendReceiveDownload($type, $buyer_id, $page)
    {
        ...
        if ($type == 'pdf') {
            $pdf = \PDF::loadView('printembrdroplets::reports.downloads.pdf.buyer-wise-print-send-receive-report-download', $data, [], [
                'format' => 'A4-L'
            ]);
            return $pdf->download('buyer-wise-print-send-receive-report.pdf');
        } else {
            return \Excel::download(new BuyerWisePrintReportExport($buyer_id), 'buyer-wise-print-send-receive-report.xlsx');

            /*\Excel::create('Buyer Wise Print Send Receive Report', function ($excel) use ($data) {
                $excel->sheet('Buyer Wise Print Report Sheet', function ($sheet) use ($data) {
                    $sheet->loadView('printembrdroplets::reports.downloads.excel.buyer-wise-print-send-receive-report-download', $data);
                });
            })->export('xls');*/
        }
    }
    ...
    {
        $buyers = Buyer::pluck('name', 'id')->all();
        return view('printembrdroplets::reports.cutting_no_wise_report')->with('buyers', $buyers);
    }
    ...
    public function getCuttingNoWisePrintReportDownload($type, $buyer_id, $purchase_order_id, $color_id, $cutting_no)
    {
        ...
        if ($type == 'pdf') {
            $pdf = \PDF::loadView('printembrdroplets::reports.downloads.pdf.cutting-no-wise-print-send-receive-report-download', $data, [], [
                'format' => 'A4-L'
            ]);
            return $pdf->download('cutting-wise-print-send-receive-report.pdf');
        } else {
            return \Excel::download(new CuttingWisePrintReportExport($data), 'cutting-wise-print-send-receive-report.xlsx');

            /*\Excel::create('Cutting No Wise Print Send Receive Report', function ($excel) use ($data) {
                $excel->sheet('Cutting Wise Print Report Sheet', function ($sheet) use ($data) {
                    $sheet->loadView('printembrdroplets::reports.downloads.excel.cutting-no-wise-print-send-receive-report-download', $data);
                });
            })->export('xls');*/
        }
    }
    ...
    public function getDateWisePrintReport(Request $request)
    {
        ...
        return view('printembrdroplets::reports.date_wise_print_factory_report', [
            'reports' => $reports,
            'from_date' => $from_date,
            'to_date' => $to_date,
        ]);
    ...
    {
        return DateWisePrintFactoryReport::with('buyer:id,name', 'order:id,order_style_no,repeat_no,booking_no,total_quantity', 'purchaseOrder:id,po_no,po_quantity', 'color:id,name', 'size:id,name','bundle_card:id,quantity,total_rejection,production_rejection_qty,qc_rejection_qty', 'factory')
            ->whereDate('production_date', '>=', $from_date)
            ->whereDate('production_date', '<=', $to_date)
            ->selectRaw('buyer_id, order_id, purchase_order_id, color_id, size_id, factory_id,bundle_card_id,
                SUM(print_received_qty) as print_received_qty_sum,
                SUM(embroidery_received_qty) as embroidery_received_qty_sum,
                SUM(production_qty) as production_qty_sum,
                SUM(qc_qty) as qc_qty_sum
            ')
            ->groupBy('buyer_id', 'order_id', 'purchase_order_id', 'color_id', 'size_id', 'factory_id', 'bundle_card_id')
            ->orderBy('buyer_id', 'asc')
            ->orderBy('order_id', 'asc')
            ->orderBy('purchase_order_id', 'asc')
            ->orderBy('color_id', 'asc')
            ->orderBy('size_id', 'asc')
            ->orderBy('bundle_card_id', 'asc')
            ->get()
            ->filter(function ($item, $key) {
                return $item->print_received_qty_sum > 0 || $item->embroidery_received_qty_sum > 0 || $item->production_qty_sum > 0 || $item->qc_qty_sum > 0;
            });
    }
    ...
    public function getDateWisePrintReportDownload(Request $request)
    {
        ...
        if ($type == 'pdf') {
            $pdf = \PDF::loadView('printembrdroplets::reports.downloads.pdf.date-wise-print-send-receive-report-download', $data, [], [
                'format' => 'A4-L'
            ]);
            return $pdf->download('date-wise-print-factory-receive-production-delivery-report.pdf');
        } else {
            return \Excel::download(new DateWisePrintFactoryReportExport($data), 'date-wise-print-receive-delivery-report.xlsx');
        }
    }
