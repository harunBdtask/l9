        ...
        <div class="single-challan-row">
          <div class="box">
            <div class="box-header text-center">
              ...
              <a class="btn btn-xs pull-right no-print" {{-- onclick="window.print();" --}} style="font-size: 17px;">
            ...
            <div class="box-body">
                      ...
                      @foreach($groupByOrder->groupBy('purchase_order_id') as $groupByPurchaseOrder)
                        @foreach($groupByPurchaseOrder->groupBy('color_id') as $groupByColor)
                          @php
                            ...
                            $cutting_nos = $groupByColor->pluck('cutting_no')->unique()->implode(',');
                          @endphp
                          <tr>
              ...
              @if($gatePassChallan)
                @php
                  $imgPath = $gatePassChallan->createdUser->signature;
                  $cutManagerImgPath = $gatePassChallan->cuttingManager->signature;
                  $imgSrc = null;
                  $cutManagerImgSrc = null;
                  if ($imgPath && Storage::disk('public')->exists($imgPath)) {
                    $imgSrc = asset('storage/'.$imgPath);
                  }
                  if ($cutManagerImgPath && Storage::disk('public')->exists($cutManagerImgPath)) {
                    $cutManagerImgSrc = asset('storage/'.$cutManagerImgPath);
                  }
                @endphp
                <table align="center" class="autorized_table">
                  <tr class="text-center" style="font-weight: bold">
                    ...
                    <td>
                      @if($cutManagerImgSrc)
                        <img src="{{ $cutManagerImgSrc }}" alt="signature" style="width: 140px; height:auto;">
                      @endif
                    </td>
                    <td></td>
                    <td></td>
