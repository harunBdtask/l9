...
@section('title', 'Print Factory Receive Scan')
@section('content')
  <div class="padding">
    <div class="row">
      <div class="col-md-12">
        <div class="box">
          <div class="box-header text-center">
            <h2>Print Factory Receive Scan</h2>
          </div>
          <div class="box-divider m-a-0"></div>
          <div class="box-body">
            @include('partials.response-message')
            <div class="text-center js-response-message"></div>
            <form id="printFactoryReciveForm" autocomplete="off">
              ...
              <div class="form-group text-center @if(count($bundle_info) == 0) hide @endif">
                <div class="text-center">
                  <a href="{{ url('/create-factory-received-tag/'.$challan_no)}}" class="btn btn-success">Create Tag</a>
                  <a href="{{ url('/create-factory-receive-challan/'.$challan_no)}}" class="btn btn-success">Create
                    Challan</a>
                </div>
              </div>
            ...
            <table class="reportTable">
              ...
              @if(isset($bundle_info))
                ...
                @foreach($bundle_info as $bundle)
                  @php
                    $bundleQuantity = $bundle->bundle_card->quantity -
                        ($bundle->bundle_card->total_rejection + $bundle->bundle_card->print_factory_receive_rejection);
...
@push('script-head')
  <script>
    $(document).on('submit', '#printFactoryReciveForm', function (e) {
      e.preventDefault();
      let message;
      const input = {};
      ...
      if (challan_no && bundleCardId.length === 9) {
        ...
        $.ajax({
          type: 'POST',
          data: input,
          url: '/print-embr-factory-receive-scan-post',
          success: function (response) {
            ...
            if (response.status === 0) {
              ...
              $('#printFactoryReciveForm').find('.hide').removeClass('hide');

              if (response.rejection_status === 1) {
                window.location.href = '/print-embr-factory-rcv-rejection?type=print&bundeId=' + bundle.id;
              }
