...
class ControlAccountWiseTrialBalance implements TrialBalanceReportInterface
{
    public $data;
    ...
    {
        return view('finance::reports.trial-balance.control_account_wise_trail_balance_report', [
                                ...
                                ->map(
                                    function ($controlAccount) use (
                                        ...
                                        return [
                                            ...
                                            'account_name' => "{$controlAccount->name} ( {$controlAccount->code} )",
            ...
            ->whereHas('account', function ($query) use ($data) {
                $query->where('type_id', $data['type_id'])->whereHas('accountInfo', function ($query) use ($data) {
                    $query->has('groupAccount')->has('controlAccount')
                        ...
                        ->where('control_account_id', $data['control_account_id']);
                });
            });
