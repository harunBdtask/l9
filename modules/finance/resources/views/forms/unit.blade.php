                    ...
                    <div class="box-body">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                {!! Form::model($unit, ['url' => $unit ? 'finance/units/'.$unit->id : 'finance/units', 'method' => $unit ? 'PUT' : 'POST', 'files' => true]) !!}
                                ...
                                <div class="form-group">
                                    <label for="fi_project_id">Project *</label>
                                    {{--                                    {!! Form::select('fi_project_id', $projects, null, ['class' => 'form-control form-control-sm select2-input', 'id' => 'fi_project_id']) !!}--}}
                                    {!! Form::select('fi_project_id', $projects ?? [], $unit->fi_project_id ?? null, ['class' => 'form-control form-control-sm select2-input', 'id' => 'fi_project_id', 'placeholder' => 'Select a Project']) !!}

                                    @if($errors->has('fi_project_id'))
                                        <span class="text-danger">{{ $errors->first('fi_project_id') }}</span>
                                    @endif
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group m-t-md">
                                    ...
                                    <a class="btn btn-danger" href="{{ url('finance/units') }}">Cancel</a>
                                </div>
                                {!! Form::close() !!}
                            </div>
                        </div>
        ...
        $(function(){
            ...
            function fetchProjects(companyId) {
                axios.get(`/finance/api/v1/fetch-company-wise-projects/${companyId}`).then((response) => {
                    ...
                    $(`#fi_project_id`).find('option').not(':first').remove();
                    let options = [];
                    ...
                    $('#fi_project_id').append(options);
                });
            }
            $('#fi_project_id').on('change', function(){
            ...
            function fetchUsers(companyId, projectId) {
                axios.get(`/finance/api/v1/fetch-project-wise-users/${companyId}/${projectId}`)
                    .then((response) => {
