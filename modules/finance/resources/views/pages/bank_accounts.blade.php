        ...
        <div class="box">
            ...
            <div class="box-body b-t">
                <div style="margin-bottom: 15px">
                    ...
                    <a class="btn btn-sm white" href="{{ url('finance/bank-accounts/create') }}">
                        <i class="glyphicon glyphicon-plus"></i> New Bank Account
                    </a>
                    @endpermission
                    <div class="pull-right" style="width: 40%">
                        <form action="{{ url('finance/bank-accounts') }}" method="GET">
                ...
                <table class="reportTable">
                    ...
                    @forelse($bank_accounts as $bank_account)
                        ...
                        <tr class="tr-height">
                            ...
                            <td class="text-left">{{ $bank_account->accountType->account_type }}</td>
                            <td class="text-left">{{ $bank_account->currency_type_id?($bank_account->currency_type_id==1?'Home':'Foreign'):null }}</td>
                            <td class="text-left">{{ $bank_account->bank->name }}</td>
                            <td class="text-left">{{ $bank_account->bank->branch_name ?? '--' }}</td>
                            ...
                            <td class="text-left">{{ $bank_account->project->project }}</td>
                            <td class="text-left">{{ $bank_account->unit->unit }}</td>
                            <td class="text-left">{{ $bank_account->status == 1 ? 'Active' : 'In-Active' }}</td>
                            <td>
                                @permission('permission_of_bank_accounts_edit')
                                <a class="btn btn-xs btn-success"
                                   href="{{ url('finance/bank-accounts/create?id=' . $bank_account->id) }}">
