<table class="reportTable" style="border: 1px solid black;border-collapse: collapse;">
  <thead>
  @if(request()->segment(2) === 'xls')
    <tr>
      <th colspan="7"> {{ sessionFactoryName() }}</th>
    </tr>
    <tr>
      <th colspan="7"> {{ sessionFactoryAddress() }}</th>
    </tr>
  @endif
  <tr>
    <th colspan="8">Section-1 : Line, Buyer, Order &amp; PO Wise
    ...
    @foreach($sewing_output_summary->groupBy('line_id') as $groupByLine)
      @foreach($groupByLine->groupBy('purchase_order_id') as $groupByOrder)
        @php
          ...
          $booking_no = $groupByOrder->first()['booking_no'] ?? '';
...
<table class="reportTable">
  <thead>
  <tr>
    <th colspan="6" style="font-size: 14px; font-weight: bold">Section-3 : Buyer, Order &amp; PO Wise Sewing
...
<table class="reportTable">
  <thead>
  <tr>
    <th colspan="7">Section-4 : Buyer, Order, PO &amp; Colour Wise Sewing
      Output Status
    </th>
  </tr>
  </thead>
