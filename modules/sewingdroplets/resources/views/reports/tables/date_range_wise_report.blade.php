...
<table class="reportTable" id="fixTable1">
  <thead>
  <tr>
    <th colspan="8" style="font-size: 14px; font-weight: bold">Section-1 : Line, Buyer, Order &amp; PO Wise
  ...
  @if(!empty($sewing_output_summary))
    ...
    @endphp
    @foreach($sewing_output_summary->sortBy('line_id')->groupBy('line_id') as $groupByLine)
...
<table class="reportTable" id="fixTable2">
  <thead>
  <tr>
    <th colspan="4" style="font-size: 14px; font-weight: bold">Section-2 : Line Wise Summary</th>
  </tr>
  </thead>
  <thead>
  <tr>
...
<table class="reportTable" id="fixTable3">
  <thead>
  <tr>
    <th colspan="5" style="font-size: 14px; font-weight: bold">Section-3 : Buyer, Order &amp; PO Wise Sewing
  ...
  @if(!empty($sewing_output_summary))
    ...
    @endphp
    @foreach($sewing_output_summary->groupBy('purchase_order_id') as $reportByOrder)
      @php
...
<table class="reportTable" id="fixTable4">
  <thead>
  <tr>
    <th colspan="7" style="font-size: 14px; font-weight: bold">Section-4 : Buyer, Order, PO &amp; Colour Wise Sewing
      Output Status
    </th>
  </tr>
  </thead>
  ...
  @if(!empty($sewing_output_summary))
    ...
    @endphp
    @foreach($sewing_output_summary->groupBy('purchase_order_id') as $groupByOrder)
