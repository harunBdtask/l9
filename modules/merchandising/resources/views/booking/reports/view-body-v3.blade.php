...
{{--                            <th>SL</th>--}}
                            ...
                            <th style="width: 30%;">Gmts. Color</th>
                            <th style="width: 12%;">Item Color</th>
...
{{--                            <th>SL</th>--}}
                            ...
                            <th style="width: 20%;">Gmts. Color</th>
                            <th style="width: 10%;">Item Color</th>
...
{{--                            <th>SL</th>--}}
                        ...
                        @foreach($trimsDetails->groupBy('item_name') as $key => $bookingDetails)
                            ...
                            @foreach($bookingDetails->pluck('details')->groupBy('color') as $color => $colorWise)
                                @foreach($colorWise->groupBy('item_size') as $itemSize => $sizeWise)
                                    @foreach($sizeWise->groupBy('size') as $gmtSize => $gmtSizeWise)
                                        @foreach($gmtSizeWise->groupBy('item_color') as $colorItem => $item)
                                            @if($item)
                                                <tr>
{{--                                                    <td>{{ $itemIndex }}</td>--}}
                                                    <td>{{ $key }}</td>
                                                    ...
                                                    <td class="text-center">{{ $item->first()['uom'] ?? ''}}</td>
                                                    @php
