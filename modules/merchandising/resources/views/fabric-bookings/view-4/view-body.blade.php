<div>
        ...
        <div>
            <table>
                <tr>
                    ...
                    <th>Booking No</th>
                    <td>{{  collect($fabricBookings->detailsBreakdown)->pluck('budget.order.reference_no')
                                ->whereNotNull()->unique()->values()->join(', ') ?? '' }}</td>
                </tr>
                ...
                <tr>
                    ...
                    @php
                        $updated_at =  collect($fabricBookings->detailsBreakdown)->sortByDesc('updated_at')
                                        ->first()['updated_at']  ?? $fabricBookings->updated_at;
                    @endphp
                ...
                <table>
                    <tr>
                        <td colspan="20" class="text-center">
                            <b>Fabric Details ({{ ($fabricDetailsUomWise)->keys()[0] }})</b></td>
                    </tr>
                    <tr>
                        <th>Part</th>
                        <th>Gmts Color</th>
                        ...
                        <th>Fin Type</th>
                        <th>Part Qty</th>
                        <th>Inspection Sample Qty.</th>
                        <th>Consmp</th>
                        <th>Unit</th>
                        <th>Shipment/Carton/Wash Sample Fab. Qty.</th>
                        <th>Act.Fab.Qty</th>
                        <th>Loss</th>
                        <th>Total Fab. Qty</th>
                        <th>Remarks</th>
                        ...
                        @foreach($fabricDetailsUomWise as $index => $details)
                            ...
                            @foreach($details as $key => $item)
                                <tr>
                                    ...
                                    <td>{{ $item['inspection_sample_qty']  ?? ''}}</td>
                                    <td>{{ round($item['fabric_consumption'], 2)  ?? ''}}</td>
                                    <td>{{ $item['uom']  ?? ''}}</td>
                                    <td class="text-right">{{ $item['sample_fabric_qty'] ?? '' }}</td>
                                    <td class="text-right">
                                        @if($item['process_loss'] > 0)
                                            @php
                                                $actual_value = $item['sample_fabric_qty'] + (($item['total_fabric_qty']/(100+$item['process_loss']))*100 ?? 0.00);
                                                $total_actual_value += $actual_value;
                                            @endphp
                                            {{ round($actual_value) }}
                                        @else
                                            @php
                                                $actual_value = $item['sample_fabric_qty'] + ($item['total_fabric_qty']  ?? 0.00);
                                                $total_actual_value += $actual_value;
                                            @endphp
                                            {{ round($actual_value) }}
                                        @endif
                                    ...
                                    @php
                                        $fabricQty = (($actual_value * $item['process_loss']) / 100) + $actual_value;
                                        $total_fabric_qty += $fabricQty;
                                    @endphp
                                    <td class="text-right">{{ round($fabricQty) }}</td>
                                    <td>{{ $item['remarks2']  ?? ''}}</td>
                                </tr>
                            @endforeach
                            <tr>
                                <th colspan="15" class="text-center">Total</th>
                                <td class="text-right">{{ collect($details)->sum('sample_fabric_qty') }}</td>
                                <td class="text-right">{{ round($total_actual_value) }}</td>
                                <td></td>
                                <td class="text-right">{{ round($total_fabric_qty) }}</td>
                                <td></td>
                            </tr>
                        @endforeach
    ...
    @if( ($fabricBookings->fabric_source == 1) && count($yarnDetails)>0)
        <div style="margin-top: 15px">
            <table>
                <tr>
                    <th colspan="9" class="text-center"><b>Yarn Details</b></th>
                </tr>
                <tr>
                    <th>Unique Id</th>
                    <th>Style</th>
