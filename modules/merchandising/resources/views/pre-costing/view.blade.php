...
@section('title','Pre Costing Report')
@section('content')
    <style type="text/css">
                    ...
                    <form action="">
                        ...
                        <div class="col-sm-6">
                            <div class="col-sm-3">
                                <div class="form-group">
                                    <label>Item</label>
                                    <select name="item_id" class="form-control select2-input" id="item_id">
                                        <option value="">Select Item</option>
                                        @foreach($items as $item)
                                            <option
                                                    value="{{ $item->id }}" {{ $item->id == $itemId ? 'selected' : null }}>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="form-group">
                                    ...
                                    <input name="from_date" id="from_date" style="height: 40px;" type="text"
                                           value="{{ $fromDate ? \Carbon\Carbon::parse($fromDate)->format('m/d/Y') : null }}"
                            ...
                            <div class="col-sm-3">
                                <div class="form-group">
                                    ...
                                    <input name="to_date" id="to_date" style="height: 40px;" type="text"
                                           value="{{ $toDate ? \Carbon\Carbon::parse($toDate)->format('m/d/Y') : null }}"
                                           class="form-control datepicker" autocomplete="off">
                                </div>
                            </div>
                            <div class="col-sm-1">
                    ...
                    <div class="">
                        <div class="header-section" style="padding-bottom: 0px;">
                            <div class="pull-right" style="margin-bottom: -5%;">
                                <a  class="btn print" href="javascript:void(0)"><i
                                            class="fa fa-print"></i>
                                </a>
                            ...
                            <table style="border: 1px solid black;width: 20%;">
                                ...
                                <tr>
                                    <td class="text-center">
                                        <span style="font-size: 12pt; font-weight: bold;">Pre Costing Report</span>
                                        <br>
                                    </td>
                                </tr>
                                </thead>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.print').click(function (e) {
                ...
                let link = `{{ url('/pre-costing-report/print') }}?factory_id=${factory_id}&buyer_id=${buyer_id}&season_id=${season_id}&from_date=${from_date}&to_date=${to_date}&style_name=${style_name}&item_id=${itemId}`;
        ...
        $(document).on('click', '#order_pdf', function () {
            ...
            let link = `{{ url('/pre-costing-report/pdf') }}?factory_id=${factory_id}&buyer_id=${buyer_id}&season_id=${season_id}&from_date=${from_date}&to_date=${to_date}&style_name=${style_name}&item_id=${itemId}`;
