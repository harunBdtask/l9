...
@section('title','Budget Wise WO Report')
@section('content')
    <style>
                ...
                <script>
                    ...
                    $(document).ready(function () {
                        ...
                        $(document).on('submit', '#reportForm', function (e) {
                            ...
                            reportTable.empty();
                            $.ajax({
                                url: "/budget-wise-wo-report/get-report",
                                type: "get",
                                dataType: "html",
                                data: formData,
                                beforeSend() {
                        ...
                        $(document).on('click', '#report_pdf', function () {
                            ...
                            let uniqueIdId = uniqueId.val();
                            let queryString = `company_id=${buyerId}&buyer_id=${companyId}&unique_id=${uniqueIdId}`

                            window.location.assign(`/budget-wise-wo-report/get-report-pdf?${queryString}`);
                        });
                        ...
                        $(document).on('click', '#report_excel', function () {
                            ...
                            window.location.assign(`/budget-wise-wo-report/get-report-excel?${queryString}`);
                        });
                    });
                    ...
                    function fetchUniqueId() {
                        ...
                        uniqueId.empty().append(`<option value="">Select Unique</option>`).val('').trigger('change');
                        $.ajax({
                            method: 'GET',
                            url: `/common-api/buyers-style-name/${buyerId}`,
                            success(result) {
                                $.each(result, function (key, value) {
                                    let element = `<option value="${value.unique_id}">${value.text}</option>`;
                                    uniqueId.append(element);
                                })
                            },
                            error: function (error) {
                                console.log(error)
