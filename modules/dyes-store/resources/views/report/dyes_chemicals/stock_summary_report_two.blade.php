    ...
    <div class="padding">
        <div class="box">
            <div class="box-header">
                <h2>Dyes & Chemicals Stock Issue Report</h2>
...
{{--                    <a id="pdf" type="button" data-toggle="tooltip" data-placement="top" title="PDF">--}}
            ...
            <div class="box-body table-responsive b-t">
                <div class="row m-b-2">
                    <form action={{ url("/dyes-store/dyes-stock-summary-report-two") }} method="GET" id="form">
                        <div class="col-md-3 form-group">
                            <input type="hidden" name="store_id" value="{{ old('store_id') ?? $storeId ?? null }}">
                            ...
                            @component('dyes-store::alert', ['name' => 'first_date']) @endcomponent
                        </div>
                        <div class="col-md-3 form-group">
                            <input type="text" class="form-control form-control-sm"
                                   value="{{ old('last_date') ?? $last_date ?? null }}" name="last_date" id="last_date"
                                   autocomplete="false" placeholder="Select To Date">
                            @component('dyes-store::alert', ['name' => 'last_date']) @endcomponent
                        </div>
                        <div class="col-md-3">
                        ...
                        <table class="reportTable" id="header-fixed">
                            ...
                            @if(isset($items))
                                ...
                                @foreach($items as $key => $item)
                                    @php
                                        $stockDetails = $item->dyesLifeDateWiseStock($first_date,$last_date, $storeId);
                                    @endphp
                                    @include('dyes-store::report.dyes_chemicals.stock_summary_report_two_table_raw',[
