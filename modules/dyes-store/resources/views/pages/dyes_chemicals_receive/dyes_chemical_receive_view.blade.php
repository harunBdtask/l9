                ...
                <div class="">
                    ...
                    <center>
                        <table style="border: 1px solid black;width: 20%;background: #02aeef">
                            ...
                            <tr>
                                <td class="text-center">
                                    <span style="font-size: 12pt; font-weight: bold;">Dyes & Chemicals Receive</span>
                                    <br>
                                </td>
                            </tr>
                            </thead>
                    ...
                    <div class="body-section">
                        <table>
                            ...
                            <tr>
                                ...
                                <th class="text-left">Lc No</th>
                                <th class="text-left">Lc Receive Date</th>
                                <th class="text-left">Pi No</th>
                                <th class="text-left">Receive Qty.</th>
                                <th class="text-left">Rate</th>
                                <th class="text-left">Total Value</th>
                                <th class="text-left">Additional Cost</th>
                                <th class="text-left">UOM</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach ($dyesChemicalReceive->details as $detail)
                                <tr>
                                    <td class="text-left">{{ $loop->iteration }}</td>
                                    <td class="text-left">{{ $dyesChemicalReceive->supplier->name ?? '' }}</td>
                                    <td class="text-left">{{ $dyesChemicalReceive->bill_no }}</td>
                                    <td class="text-left">{{ $detail['item_name'] ?? '' }}</td>
                                    <td class="text-left">{{ $detail['category_name'] ?? '' }}</td>
                                    <td class="text-left">{{ $detail['brand_name'] ?? '' }}</td>
                                    <td class="text-left">{{ \Carbon\Carbon::parse($dyesChemicalReceive->receive_date)->toFormattedDateString() }}</td>
                                    <td class="text-left">{{ $dyesChemicalReceive->lc_no ?? '' }}</td>
                                    <td class="text-left">{{ \Carbon\Carbon::parse($dyesChemicalReceive->lc_receive_date)->toFormattedDateString() }}</td>
                                    <td class="text-left"></td>
                                    <td class="text-left">{{ $detail['receive_qty'] ?? '' }}</td>
                                    <td class="text-left">{{ $detail['rate_taka'] ?? '' }}</td>
                                    <td class="text-left">{{ ($detail['receive_qty'] * $detail['rate_taka']) ?? '' }}</td>
                                    <td class="text-left">{{ $detail['details']['additional_cost'] ?? '' }}</td>
