...
@section('title','User Wise Buyer Permission')
@section('styles')
    <style>
    ...
    <div class="padding">
        <div class="row">
            <div class="col-md-7 col-md-offset-2">
                <div class="box assign_permission_box form-colors">
                    <div class="box-header">
                        <h2>{{  'User Wise Buyer Permission' }}</h2>
                    </div>
                    ...
                    <div class="box-body">
                        ...
                        {!! Form::open(['url' => 'user-wise-buyer-permission', 'method' => 'post', 'id' => 'user-wise-buyer-permission-form']) !!}

                        <div class="form-group">
                            <label for="factory_id">Select
                                Company</label>
                        ...
                        <div class="form-group">
                            <label for="buyer_id">Select Buyers</label>
                            {!! Form::select('buyer_id[]', [], null, ['class' => 'form-control', 'multiple' => true]) !!}
                        ...
                        <div class="form-group">
                            <label for="buyer_id">View Buyers</label>
                            {!! Form::select('view_buyer_id[]', [], null, ['class' => 'form-control', 'multiple' => true]) !!}
                            <span class="text-danger buyer_id"></span>
                            @if($errors->has('view_buyer_id'))
                                <span class="text-danger">{{ $errors->first('view_buyer_id') }}</span>
                            @endif
                        </div>
                        ...
                        <div class="form-group">
                            <button type="submit"
                                    class="btn btn-sm btn-suceess"><i class="fa fa-save"></i> Save
                            </button>
                        </div>
                        {!! Form::close() !!}
                    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('change', 'select[name="factory_id"]', function () {
                ...
                var buyersDomElements = $('select[name="buyer_id[]"]');
                var viewBuyersDomElements = $('select[name="view_buyer_id[]"]');
                buyersDomElements[0].innerHTML = '';
                if (factory_id) {
                    $.ajax({
                        type: 'GET',
                        url: '/get-buyers/' + factory_id,
                        success: function (response) {
                            factoryWiseBuyerDom(buyersDomElements, response);
                            factoryWiseBuyerDom(viewBuyersDomElements, response);
                        }
                    });
                }
            });

            function factoryWiseBuyerDom(buyersDomElements, response) {
                $.each(buyersDomElements, function (index, domElement) {
                    domElement.innerHTML = "";
                    var dropdown = '';
                    dropdown += '<option value="all_buyer">All Buyer</option>';
                    if (Object.keys(response).length > 0) {
                        $.each(response, function (index, val) {
            ...
            function loadAssignFullPermissionPage() {
                window.location.href = '/user-wise-buyer-permission';
            }
            ...
            $(document).on('click', 'button.remove_all_permission', function (e) {
                ...
                if (confirmMessage) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: '/remove-module-wise-full-permission',
                        data: data
                    }).done(function (response) {
                        loader.hide();
                        if (response.status == 'success') {
