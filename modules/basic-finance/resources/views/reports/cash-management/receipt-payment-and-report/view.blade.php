...
@section('title', 'Cash Receipt and Payment Statement Report')
@section('styles')
    <style type="text/css">
        .addon-btn-primary {
            padding: 0;
    ...
    <div class="padding">
        <div class="box">
            <div class="box-header">
                <h2>Cash Receipt and Payment Statement Report</h2>
            </div>
                                ...
                                <div class="col-sm-3">
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="start_date"
                                               value="{{ empty(request('start_date')) ? Carbon\carbon::today()->startOfMonth() : request('start_date') }}"
                                               class="form-control select2-input" id="start_date">
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="col-sm-5">
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="end_date"
                                               value="{{ empty(request('end_date')) ? Carbon\carbon::today()->endOfMonth() : request('end_date') }}"
                                               class="form-control select2-input" id="end_date">
                                    </div>
                                </div>
                                <div class="col-sm-5">
                                    <div class="form-group">
                                        <label>Accounts</label>
                                        <select name="account_ids" class="form-control select2-input" multiple id="account_ids">
                                            <option value="">Select Account</option>
                                            @foreach($account_ids as $account)
                                                <option
                                                    value="{{ $account->id }}" {{ $account->id == $accountIds ? 'selected' : null }}>{{ $account->text }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                ...
                <div class="">
                    <div class="header-section" style="padding-bottom: 0px;">
                        <div class="pull-right" style="margin-bottom: -5%;">
                            <a class="btn"
                               href="/basic-finance/cash-receipts-and-payments?{{ Request::getQueryString().'&type=pdf' }}"><i
                        ...
                        <table style="border: 1px solid black;width: 30%;">
                            ...
                            <tr>
                                <td class="text-center">
                                    <span style="font-size: 12pt; font-weight: bold;">Cash Receipt and Payment Statement Report</span>
                                    <br>
                                </td>
                            </tr>
                            </thead>
                    ...
                    <div class="body-section" style="margin-top: 0;">
                        <div class="row">
                            <div class="col-lg-12">
                                                                @includeIf('basic-finance::reports.cash-management.receipt-payment-and-report.table')
                            </div>
                        </div>
                    </div>
...
@section('scripts')
    <script type="text/javascript">
        ...
        $(function () {
            ...
            const accountsElement = $(`#account_ids`);
            function fetchAccounts(companyId) {
                axios.get(`/basic-finance/api/v1/fetch-company-wise-cash-accounts/${companyId}`)
                    .then((response) => {
                        accountsElement.find('option').remove();
                        ...
                        accountsElement.append(options);
                        accountsElement.select2('val', []);
