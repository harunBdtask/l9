...
class MonthlyPaymentSummaryRepository
{
    public function generateMonthlyPaymentSummary($request)
    {
        try {
            $month = $request->month ?? null;
            $year = $request->year ?? null;
            $unique_id = $request->employee_id ?? null;
            DB::beginTransaction();
    ...
    private function calculateMonthlyPaymentSummary($month, $year, $employee, $total_working_day, $total_holiday)
    {
        ...
        $punch_card_id = $employee->punch_card_id;
        ...
        $ot_minute = $this->calculateEmployeeRegularOtMinute($employee_id, $userid, $punch_card_id, $month, $year, $employee_type);
        ...
        $night_ot_minute = $this->calculateEmployeeNightOtMinute($employee_id, $userid, $punch_card_id, $month, $year);
        ...
        $absent_deduction = round((($basic_salary * $total_absent_day) / $pay_month->copy()->daysInMonth), 3);
        $attendance_bonus_deduction = $this->calculateAttendanceBonusDeduction($attendance_bonus, $total_leave, $total_absent_day, $employee_id, $userid, $month, $year, $total_working_day,$total_present_day );
//        $revenue_stamp = 10;
        $revenue_stamp = 0;
     ...
     * @return mixed
     */
    private function calculateEmployeeRegularOtMinute($employee_id, $userid, $punch_card_id, $month, $year, $employee_type)
    {
        if ($employee_type == 'staff') {
            return 0;
        }
...
//            ])->sum('regular_ot_minute');
            ->where([
                'present_status' => 1,
                'userid' => $punch_card_id,
            ])
            ->sum('total_in_day_ot_minute');
    }
     ...
     * @return float
     */
    private function calculateAttendanceBonusDeduction($attendance_bonus, $total_leave, $total_absent_day, $employee_id, $userid, $month, $year,$total_working_day,$total_present_day)
    {
        if($total_absent_day>0){
            return $attendance_bonus;
        }
        elseif ($total_working_day == $total_present_day){
            return 0;
        }
        elseif($total_absent_day==0 && $total_leave == 1 ){
            return 0;
        }
        elseif($total_absent_day == 0 && $total_leave == 2){
            if($attendance_bonus>100){
                return 100;
            }
            return $attendance_bonus;
        }
...
//        }
     ...
     * @return mixed
     */
    private function calculateEmployeeNightOtMinute($employee_id, $userid, $punch_card_id, $month, $year)
    {
        $date = Carbon::parse($year . '-' . $month . '-01');
        $start_date = $date->copy()->startOfMonth()->toDateString();
        $end_date = $date->copy()->endOfMonth()->toDateString();
