...
use SkylarkSoft\GoRMG\Subcontract\Services\SubSlittingService;
use SkylarkSoft\GoRMG\SystemSettings\Models\Buyer;
use SkylarkSoft\GoRMG\SystemSettings\Models\Shift;

class SubSlitting extends Model
{
    use SoftDeletes;
    use CommonModelTrait;
    use BelongsToFactory;

    protected $table = 'sub_slittings';
    ...
    public static function booted()
    {
        static::saving(function ($model) {
            if (! $model->id && in_array('created_by', $model->getFillable())) {
                $model->sub_slitting_uid = SubSlittingService::generateUniqueId();
            }
        });
    }
    ...
    public function scopeSearch(Builder $query, Request $request)
    {
        $query->when($request, function (Builder $query) use ($request) {
            $query->when(
                ...
                return $query->whereHas('subSlittingDetails', function (Builder $query) use ($request) {
    ...
    public function subSlittingDetails(): HasMany
    {
        return $this->hasMany(SubSlittingDetail::class, 'sub_slitting_id');
    }
