...
use SkylarkSoft\GoRMG\Subcontract\Services\SubDyeingSqueezerService;
use SkylarkSoft\GoRMG\SystemSettings\Models\Buyer;
use SkylarkSoft\GoRMG\SystemSettings\Models\Color;
use SkylarkSoft\GoRMG\SystemSettings\Models\Shift;

class SubDyeingSqueezer extends Model
{
    use SoftDeletes;
    use CommonModelTrait;
    use BelongsToFactory;

    protected $table = "sub_dyeing_squeezers";
    ...
    public function scopeSearch($query, Request $request)
    {
        ...
        return $query->when($factoryId, Filter::applyFilter('factory_id', $factoryId))
            ...
            ->when($color, function ($query) use ($color) {
                $query->whereHas('subDyeingSqueezerDetail', function ($q) use ($color) {
                    return $q->where('color_id', $color);
                });
            })->when($request->input('finish_qty'), function (Builder $query) use ($request) {
                return $query->whereHas('subDyeingSqueezerDetail', function (Builder $query) use ($request) {
    ...
    public static function booted()
    {
        static::saving(function ($model) {
            $model->squeezer_uid = SubDyeingSqueezerService::generateUniqueId();
        });
    }
    ...
    {
        return $this->hasMany(SubDyeingSqueezerDetail::class, 'sub_dyeing_squeezer_id');
    }
