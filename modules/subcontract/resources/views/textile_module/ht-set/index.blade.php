            ...
            <div class="box-body">
                ...
                <div class="row">
                    <div class="col-md-12 m-b">
                        <a href="{{ url('subcontract/ht-set/create') }}"
                ...
                <div class="row">
                    <div class="col-md-12">
                        <table class="reportTable">
                            ...
                            <tbody>
                            {!! Form::open(['url'=>'/subcontract/ht-set', 'method'=>'GET']) !!}
                            <tr>
                                <td>*</td>
                                <td>
                            ...
                            @forelse ($subDyeingHtSets as $htSet)
                                @php
                                    $colors = collect($htSet->subDyeingHtSetDetail)->pluck('color')->pluck('name')->join(', ');
                                    $totalOrderQty = collect($htSet->subDyeingHtSetDetail)->sum('order_qty');
                                @endphp
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $htSet->entry_basis_value }}</td>
                                    <td>{{ $htSet->production_date }}</td>
                                    <td>{{ $htSet->factory->factory_name }}</td>
                                    <td>{{ $htSet->supplier->name }}</td>
                                    <td>{{ $htSet->sub_textile_order_no ?? 'N\A' }}</td>
                                    <td>{{ $htSet->sub_dyeing_batch_no ?? 'N\A' }}</td>
                                    <td>{{ $htSet->shift->shift_name }}</td>
                                    <td>{{ $colors }}</td>
                                    <td>{{ $totalOrderQty }}</td>
                                    <td>{{ $htSet->machine->name }}</td>
                                    <td>{{ $htSet->subDyeingHtSetDetail->pluck('finish_qty')->join(', ') }}</td>
                                    <td>
                                        <a class="btn btn-xs btn-info" type="button"
                                           href="{{ url('subcontract/ht-set/create?id='. $htSet->id) }}">
                                        ...
                                        <button style="margin-left: 2px;" type="button"
                                                ...
                                                ui-target="#animate"
                                                data-url="{{ url('subcontract/ht-set/'. $htSet->id) }}">
                        ...
                        <div class="text-center">
                            {{ $subDyeingHtSets->appends(request()->except('page'))->links() }}
                        </div>
                    </div>
                </div>
            </div>
