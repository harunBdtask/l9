    ...
    @if(!empty($date_wise_input))
        ...
        @foreach($date_wise_input->sortBy('line.sort')->groupBy('floor_id') as $reportByFloor)
            ...
            @foreach($reportByFloor as $report)
                ...
                @endphp
                <tr>
                    <td style="text-align: left; padding-left: 5px;">{{ $floor_no }}</td>
                    <td style="text-align: left; padding-left: 5px;">{{ $line_no }}</td>
                    <td style="text-align: left; padding-left: 5px;">{!! $challan_no !!}</td>
                    <td style="text-align: left; padding-left: 5px;">{{ $buyer_name }}</td>
                    <td style="text-align: left; padding-left: 5px;">{{ $report->order->job_no }}</td>
                    <td style="text-align: left; padding-left: 5px;">{{ $style_name }}</td>
                    <td style="text-align: left; padding-left: 5px;" title="{{ $po_no }}">{{ substr($po_no, -30) }}</td>
                    <td>{{ $po_qty }}</td>
                    <td>{{ $color }}</td>
                    <td>{{ $input }}</td>
                    {{--  <td>{{ $input_time }}</td> --}}
            ...
            @endforeach
            <tr class="yellow-200">
                <th colspan="9">Total = {{ $floor_no }}</th>
