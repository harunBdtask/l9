<table class="reportTable" style="border: 1px solid black;border-collapse: collapse;">
    ...
    @endif
    <tr>
        <th colspan= {{ request()->segment(2) === 'xls' ? "9" : "10" }}>Section-1 : Challan No. Wise Sewing Input Status</th>
    </tr>
    <tr>
        <th>Unit</th>
        <th>Line</th>
    ...
    @if(!empty($date_wise_input))
        @php
            ...
            $input_time = '';
        @endphp
        @foreach($date_wise_input->sortBy('line_id') as $report)
            @php
                $floor_no = $report->floor->floor_no;
...
<table class="reportTable" style="border-collapse: collapse;">
    ...
    @if(!empty($date_wise_input))
        @php
            $total_input_line = 0;
        @endphp
        @foreach($date_wise_input->sortBy('line_id')->groupBy('line_id') as $report_line_wise)
            @php
                $floor_no = $report_line_wise->first()->floor->floor_no ?? '';
