      ...
      <div class="col-md-12">
        <div class="box">
          ...
          <div class="box-body">
            ...
            <form method="POST" action="{{ url('/create-challan-sewing-input')}}" accept-charset="UTF-8"
                  onsubmit="submit.disabled = true; return true;">
              @csrf
              <input type="hidden" name="challan_no" value="{{ $cutting_inventory->challan_no ?? '' }}">
              <input type="hidden" name="color_id" value="{{ $cutting_inventory->bundlecard->color_id ?? '' }}">
              ...
              <div class="form-group row">
                <div class="col-sm-4 col-sm-offset-4">
                  @php
                    if (old('floor_id')) {
                      $lines = \SkylarkSoft\GoRMG\Inputdroplets\Models\Line::where('floor_id', old('floor_id'))
                        ->pluck('line_no', 'id')
                        ->all();
                    }
                  @endphp
